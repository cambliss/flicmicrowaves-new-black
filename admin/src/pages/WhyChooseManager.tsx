...
const defaultItem = (): WhyChooseItem => ({ title: '', description: '' });

export default function WhyChooseManager() {
  const [heading, setHeading] = useState('');
  const [subtitle, setSubtitle] = useState('');
  const [items, setItems] = useState<WhyChooseItem[]>([defaultItem(), defaultItem(), defaultItem(), defaultItem()]);
  const [savedHeading, setSavedHeading] = useState('');
  const [savedSubtitle, setSavedSubtitle] = useState('');
  const [savedItems, setSavedItems] = useState<WhyChooseItem[]>([]);
  ...
  const loadContent = async () => {
    ...
    setError('');
    try {
      const { data } = await client.get('/api/home-content/why-choose');
      setHeading(data.heading || '');
      setSubtitle(data.subtitle || '');
      setItems(data.items?.length ? data.items : [defaultItem(), defaultItem(), defaultItem(), defaultItem()]);
  ...
  const handleItemChange = (index: number, key: keyof WhyChooseItem, value: string) => {
    setItems((prev) => {
      const next = [...prev];
  ...
  const handleSave = async (event: React.FormEvent) => {
    ...
    if (!heading.trim() || !subtitle.trim() || cleanedItems.length === 0) {
      setError('Heading, subtitle, and at least one item are required');
      return;
    }

    setSaving(true);
    try {
      await client.put('/api/home-content/why-choose', {
  ...
  const handleClearSavedSection = async () => {
    if (!window.confirm('Delete all saved Why Choose content?')) return;
  ...
  return (
    <div style={{ maxWidth: 1100, margin: '0 auto', padding: '32px 16px' }}>
      <div style={{ display: 'grid', gap: 14, marginBottom: 28, justifyItems: 'start' }}>
        ...
        <p style={{ color: '#6c757d', fontSize: 13 }}>Manage Why Choose section content</p>
      ...
      {!loading && (
        <div style={{ display: 'grid', gap: 18 }}>
          <form className="card" style={{ display: 'grid', gap: 14 }} onSubmit={handleSave}>
            <h2 style={{ fontSize: 18, fontWeight: 700, color: '#1a1a1a' }}>Why Choose Section</h2>
          ...
          <input
            ...
            placeholder="Why Choose Flic Microwaves?"
          />
          ...
          <textarea
            ...
            placeholder="Trusted by defence, aerospace..."
          />
