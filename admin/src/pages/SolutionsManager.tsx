...
const defaultItem = (): SolutionItem => ({
  title: '',
  description: '',
...
export default function SolutionsManager() {
  ...
  const [items, setItems] = useState<SolutionItem[]>([defaultItem(), defaultItem(), defaultItem()]);

  const [savedHeading, setSavedHeading] = useState('');
  const [savedSubtitle, setSavedSubtitle] = useState('');
  const [savedItems, setSavedItems] = useState<SolutionItem[]>([]);
  ...
  const loadContent = async () => {
    ...
    setError('');
    try {
      const { data } = await client.get('/api/home-content/solutions');
  ...
  const handleItemChange = (index: number, key: keyof SolutionItem, value: string) => {
    setItems((prev) => {
      const next = [...prev];
  ...
  const handleSave = async (event: React.FormEvent) => {
    ...
    if (!heading.trim() || !subtitle.trim() || cleanedItems.length === 0) {
      setError('Heading, subtitle, and at least one solution card are required');
      return;
    }

    setSaving(true);
    try {
      await client.put('/api/home-content/solutions', {
  ...
  const handleDeleteSavedItem = async (index: number) => {
    if (!window.confirm('Delete this saved solution card?')) return;
    ...
    try {
      ...
      setError('Failed to delete saved solution card');
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleClearSavedSection = async () => {
    if (!window.confirm('Delete all saved solutions content?')) return;
  ...
  return (
    <div style={{ maxWidth: 1100, margin: '0 auto', padding: '32px 16px' }}>
      <div style={{ display: 'grid', gap: 14, marginBottom: 28, justifyItems: 'start' }}>
        ...
        <p style={{ color: '#6c757d', fontSize: 13 }}>Manage Our Solutions section content</p>
      ...
      {!loading && (
        <div style={{ display: 'grid', gap: 18 }}>
          <form className="card" style={{ display: 'grid', gap: 14 }} onSubmit={handleSave}>
            <h2 style={{ fontSize: 18, fontWeight: 700, color: '#1a1a1a' }}>Our Solutions Section</h2>
            ...
            <textarea
              ...
              placeholder="Comprehensive electronic solutions..."
            />
            ...
            <div>
              <button type="submit" className="btn btn-primary" disabled={saving}>
                {saving ? 'Saving...' : 'Save Our Solutions Section'}
              </button>
            </div>
          </form>
