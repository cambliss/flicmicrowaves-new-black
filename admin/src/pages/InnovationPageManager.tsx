...
const sections: LinkSection[] = [
  { slug: 'hero', title: 'Hero', desc: 'Main heading, subtitle, and CTAs.' },
  { slug: 'focus-areas', title: 'Focus Areas', desc: 'Innovation focus area cards.' },
  { slug: 'lab-capabilities', title: 'Lab Capabilities', desc: 'Lab capability bullet points.' },
  { slug: 'pipeline', title: 'Pipeline', desc: 'Innovation workflow steps.' },
  { slug: 'metrics', title: 'Metrics', desc: 'Performance and activity stats.' },
  { slug: 'featured-projects', title: 'Featured Projects', desc: 'Case-style project highlights.' },
...
const slugToApi: Record<string, string> = {
  ...
  'lab-capabilities': 'labCapabilities',
  pipeline: 'pipeline',
  metrics: 'metrics',
...
function sectionPath(slug: string) {
  return slug === 'hero' ? '/innovation-page-content' : `/innovation-page-content/${slug}`;
}

const emptyPair = () => ({ title: '', body: '' });
const emptyMetric = () => ({ value: '', label: '' });
const emptyProject = () => ({ title: '', problem: '', approach: '', outcome: '' });
const emptyPipeline = () => ({ step: '', title: '', body: '' });

export default function InnovationPageManager() {
  const location = useLocation();
  const routeSlug = location.pathname.split('/').filter(Boolean).pop() || 'innovation-page-content';
  const slug = routeSlug === 'innovation-page-content' ? 'hero' : routeSlug;
  ...
  const loadSection = async () => {
    ...
    setSuccess('');
    try {
      const { data } = await client.get(`/api/home-content/innovation-page/${apiSection}`);
      setForm(data || {});
    } catch {
  ...
  const saveSection = async (event: React.FormEvent) => {
    ...
    try {
      await client.put(`/api/home-content/innovation-page/${apiSection}`, form);
  ...
  const resetSection = async () => {
    ...
    setSuccess('');
    try {
      await client.delete(`/api/home-content/innovation-page/${apiSection}`);
  ...
  const resetAllSections = async () => {
    const confirmed = window.confirm('Reset ALL Innovation sections to defaults? This will clear all saved innovation page content.');
    if (!confirmed) return;
    ...
    setSuccess('');
    try {
      await client.delete('/api/home-content/innovation-page');
      setSuccess('All Innovation sections reset to defaults');
      await loadSection();
    } catch {
  ...
  return (
    <div style={{ maxWidth: 1200, margin: '0 auto', padding: '32px 16px' }}>
      <div style={{ display: 'grid', gap: 10, marginBottom: 20 }}>
        <h1 style={{ fontSize: 24, fontWeight: 700, color: '#b8860b' }}>Flic Microwaves - Innovation Page CMS</h1>
        <p style={{ color: '#6c757d', fontSize: 13 }}>
          Manage Innovation page one section at a time. This is fully dynamic and connected to backend APIs.
        </p>
        <div>
          <button type="button" className="btn btn-danger" onClick={resetAllSections} disabled={saving}>
            Reset All Innovation Sections
          </button>
        </div>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '300px 1fr', gap: 18 }}>
        <aside className="card" style={{ display: 'grid', gap: 8, alignContent: 'start', height: 'fit-content' }}>
          <h2 style={{ fontSize: 16, fontWeight: 700, color: '#1a1a1a', marginBottom: 4 }}>Innovation Sections</h2>
...
function renderSectionEditor(
  ...
  if (slug === 'focus-areas') {
    ...
    return renderPairEditor('Focus Areas', items, (next) => setSection(next), 'title', 'body');
  }

  if (slug === 'lab-capabilities') {
    const items = Array.isArray(form) ? form : ['', '', ''];
    return renderStringListEditor('Lab Capabilities', items, (next) => setSection(next));
  }

  if (slug === 'pipeline') {
    const items = Array.isArray(form) ? form : [emptyPipeline(), emptyPipeline()];
    return renderTripleEditor('Pipeline Steps', items, (next) => setSection(next), 'step', 'title', 'body');
  }
  ...
  if (slug === 'featured-projects') {
    const items = Array.isArray(form) ? form : [emptyProject(), emptyProject()];
    return renderQuadEditor('Featured Projects', items, (next) => setSection(next), 'title', 'problem', 'approach', 'outcome');
  }
