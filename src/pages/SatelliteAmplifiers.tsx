...
import {
  ArrowLeft, ArrowRight, Filter, Satellite, CheckCircle, Mail, Phone,
...
const typeOptions = [
  ...
  { value: 'Driver', label: 'Driver Amplifier' },
  { value: 'SSPA', label: 'Solid State PA' },
  { value: 'TWTA', label: 'Traveling Wave Tube' },
];

const frequencyOptions = [
  { value: 'all', label: 'All Frequencies' },
  { value: 'L-Band', label: 'L-Band (1-2 GHz)' },
  { value: 'S-Band', label: 'S-Band (2-4 GHz)' },
  { value: 'C-Band', label: 'C-Band (4-8 GHz)' },
  { value: 'X-Band', label: 'X-Band (8-12 GHz)' },
  { value: 'Ku-Band', label: 'Ku-Band (12-18 GHz)' },
  { value: 'Ka-Band', label: 'Ka-Band (26-40 GHz)' },
];

const SatelliteAmplifiers = () => {
  ...
  const filteredProducts = products.filter((p) => {
    ...
    const typeMatch =
      selectedType === 'all' ||
      typeSpec.includes(selectedType.toLowerCase()) ||
      nameAndDesc.includes(selectedType.toLowerCase());
    const freqMatch =
      selectedFrequency === 'all' ||
      freqSpec.includes(selectedFrequency.toLowerCase()) ||
      nameAndDesc.includes(selectedFrequency.toLowerCase());
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 w-full text-left py-20">
          <div className="max-w-3xl">
            <h1 className="text-4xl md:text-5xl font-bold text-white mb-4">
              Satellite <span className="text-black">Amplifiers</span>
            </h1>
            <p className="text-xl text-white/90 leading-relaxed font-opensans">
              High-performance amplifiers engineered for satellite communications, earth stations,
              and aerospace applications. Delivering exceptional gain and reliability.
            </p>
          </div>
        </div>
      </section>
          ...
          <div className="grid lg:grid-cols-4 gap-8">
            ...
            <div className="lg:col-span-1">
              <div className="bg-white p-6 rounded-2xl border border-goldenrod/20 shadow-lg sticky top-24">
                ...
                {/* Type */}
                <div className="mb-6">
                  <h3 className="text-lg font-semibold text-black mb-3">Amplifier Type</h3>
                ...
                {/* Frequency */}
                <div className="mb-6">
                  <h3 className="text-lg font-semibold text-black mb-3">Frequency Band</h3>
                  <div className="space-y-2">
      ...
      <div className="w-full h-40 bg-goldenrod/5 overflow-hidden">
        {imgUrl ? (
          ...
          <div className="w-full h-full flex items-center justify-center">
            <Satellite className="w-12 h-12 text-goldenrod/30" />
          </div>
        )}
      </div>
      <div className="p-6">
        ...
        {product.description && (
          <p className="text-black/70 text-sm leading-relaxed mb-4 line-clamp-3 font-opensans">{product.description}</p>
        )}
...
export default SatelliteAmplifiers;
